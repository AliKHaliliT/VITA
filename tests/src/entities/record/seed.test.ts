...
describe("loadInitialData: item shape", () => {
  ...
  // The seed is the demo record: every folder ships at least one entry that
  // documents the behavior it exercises, so every type must load non-empty.
  it("returns a non-empty array for every content folder", () => {
  ...
  it("posts: frontmatter `type` maps to postType; the internal type stays 'posts'", () => {
    ...
    const list = posts.find((p) => p.slug === "reading-log") as Loose;
