...
describe("per-shelf ordering", () => {
  // The demo seed sets media/game to alphabetical; this pins that coupling.
  it("a shelf with a seeded policy reorders behind its pins", () => {
    const [shelf] = buildShelves(
      [],
      [
    ...
    expect(shelf.items.map((i) => i.slug)).toEqual(["keep", "anvil", "zeppelin"]);
  });
