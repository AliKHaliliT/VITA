# 0024. Name the adversary and let the machine catch the rest
...
The toolchain gains the mechanical security rules, the core
rules against `eval` and its indirect forms and against javascript URLs, and
the sonarjs rules for dynamic code execution, hardcoded passwords and secrets,
clear-text protocols, cryptographically weak randomness, weak ciphers and
keys, weak hashing, insecure cookies, and intrusive permission requests.
...
blind. The rules were run against the whole tree before adoption and reported
nothing, so no exclusion was needed anywhere. Each rule was proved against a planted defect before the
