# 0015. Prefer double quotes where the choice is free
...
Enforcement rides the Lint verb where a checker exists. Here ESLint's core `quotes` and `jsx-quotes` rules carry it with `avoidEscape` on. Those two are deprecated in ESLint's core in favor of the stylistic plugin, and they still function through ESLint 9; when ESLint 10 retires them, the same two rules move to `@stylistic/eslint-plugin`, which is the maintained home.
