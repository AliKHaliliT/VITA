...
Three heuristics moved from error to warning, the two that guess at hardcoded
credentials and the one that guesses whether a regular expression is slow. None
can decide its question, and the lint command does not fail on warnings, so the
demotion needed no other plumbing. Their findings still print and still oblige an
answer.
