# 0013. Bound the rot in living documents
...
The mechanical half is checked rather than reviewed. `scripts/audit-docs.mjs` verifies that every repository path a living document names exists, that every relative link resolves, and that no STATE entry has outlived the horizon; CI runs it as the Docs step before anything installs. Decision records are exempt from all of it, since they describe the past.
