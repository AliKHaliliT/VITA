...
- **The layer rule is absolute.** Imports point downward through
  ...
  down a layer. See [decision 0007, Build the site as one-way sliced layers](docs/decisions/0007-build-the-site-as-one-way-sliced-layers.md).
- **Content is checked at the door.** Everything entering the record passes through
  ...
  with its key named in a notice only that browser sees. See
  [decision 0009, Guard the record with hand-written validators](docs/decisions/0009-guard-the-record-with-hand-written-validators.md)
  and [decision 0025, Name a refused saved copy on the page](docs/decisions/0025-name-a-refused-saved-copy-on-the-page.md).
...
- **Suites mirror the source tree and substitute only at a seam.** One suite per unit under
  ...
  [decision 0013, Adopt the style's test contract](docs/decisions/0013-adopt-the-styles-test-contract.md).
...
- **Never use `type` as a frontmatter key**, because it collides with the internal
  `ContentType` field. Each collection names its kind with a key of its own:
  ...
  | `posts` | `type`, the one exception, which the loader remaps to `postType` |
